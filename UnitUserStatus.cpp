...
void __fastcall TFormUserStatus::FormShow(TObject *Sender)
{
	...
	for(unsigned i=0; i < TShopUnits::VShopUnits.size(); i++) {
		ComboBoxShopUnits->Items->Add(TShopUnits::VShopUnits[i]->Name);
	}
...
void __fastcall TFormUserStatus::ComboBoxShopUnitsChange(TObject *Sender)
{
	...
	if(ComboBoxShopUnits->ItemIndex != -1) {
		...
		FormShop->ShopUnit = TShopUnits::VShopUnits[ComboBoxShopUnits->ItemIndex];
...
*/
	...
	for(unsigned i=0; i < TWorkers::VWorkers.size(); i++) {
		if(TWorkers::VWorkers[i]->idUnitShop == FormShop->ShopUnit->id) {

			if(EditLogin->Text == TWorkers::VWorkers[i]->Login && EditPassword->Text == TWorkers::VWorkers[i]->Password) {
					FormShop->Worker = TWorkers::VWorkers[i];
