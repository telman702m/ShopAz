...
//	VSelectedProd.clear();       // clear selected product vector in Sale
	...
	ExecSQL_Managed<TMoveProduct>(FDQuery, uQuery, NameTableDB, TabDB::TB_MOVEP, TRecordType::RT_DELETE, LOGS::LG_DELETE);
}
//---------------------------------------------------------------------------
bool __fastcall TMoveProduct::SaveDataToFile(char *FileNmae)
{
	...
	for(unsigned i=0; i < TObjectManager<TMoveProduct>::GetList().size(); i++) {
		fwprintf(fs, L"'%d'\t'%d'\t'%d'\t'%d'\t'%d'\t'%d'\t'%s'\t'%d'\t'%d'\t'%d'\t'%d'\t'%s'\t'%d'\n",
					 TObjectManager<TMoveProduct>::GetList()[i]->id,
					 TObjectManager<TMoveProduct>::GetList()[i]->MoveType,
					 TObjectManager<TMoveProduct>::GetList()[i]->idSource,
					 TObjectManager<TMoveProduct>::GetList()[i]->idSourcePerson,
					 TObjectManager<TMoveProduct>::GetList()[i]->idTarget,
					 TObjectManager<TMoveProduct>::GetList()[i]->idTargetPerson,
					 TObjectManager<TMoveProduct>::GetList()[i]->DateTime.FormatString(L"dd.mm.yyyy hh:nn:ss").w_str(),
					 TObjectManager<TMoveProduct>::GetList()[i]->Discount.GetMoney(),
					 TObjectManager<TMoveProduct>::GetList()[i]->Discount.GetCent(),
					 TObjectManager<TMoveProduct>::GetList()[i]->ActualPayment.GetMoney(),
					 TObjectManager<TMoveProduct>::GetList()[i]->ActualPayment.GetCent(),
					 TUtills::ReplaceSymbol(TUtills::ReplaceSymbol(TObjectManager<TMoveProduct>::GetList()[i]->Description.w_str(), L'\r', wchar_t(27)), L'\n', wchar_t(28)),
					 TObjectManager<TMoveProduct>::GetList()[i]->bDeleted);
	}
...
{
	TObjectManager<TMoveProduct>::GetList().clear();
	...
	while(!bSuccess) {
		try {
			...
			uTmp = uTmp.sprintf(wLostConnection[iLang], L"Select", NameTableDB);
...
{
	for(unsigned i=0; i < TObjectManager<TMoveProduct>::GetList().size(); i++) {
		if(TObjectManager<TMoveProduct>::GetList()[i]->id == id) {
			return true;
		}
	}
	return false;
...
void __fastcall TMoveProduct::CalcUnitsBalance(void)
{
	...
	for(unsigned i=0; i < CountProduct; i++) {
		for(unsigned j=0; j < CountShop; j++) {
			TObjectManager<TProduct>::GetList()[i]->CountBalance[j] = 0;
		}
	}

	unsigned CountMove = TObjectManager<TMoveProduct>::GetList().size();

	for(unsigned i=0; i < CountMove; i++) {
		TObjectManager<TMoveProduct>::GetList()[i]->CalcObjectUnitsBalance();
	}
...
{
	unsigned CountBuyer = TObjectManager<TBuyer>::GetList().size();
	for(unsigned i=0; i < CountBuyer; i++) {
		TObjectManager<TBuyer>::GetList()[i]->SetPricesZero();
	}

	unsigned CountProvisioner = TObjectManager<TProvisioner>::GetList().size();
	for(unsigned i=0; i < CountProvisioner; i++) {
		TObjectManager<TProvisioner>::GetList()[i]->SetPricesZero();
	}

	unsigned CountShop = TObjectManager<TShopUnits>::GetList().size();
	for(unsigned i=0; i < CountShop; i++) {
		TObjectManager<TShopUnits>::GetList()[i]->SetPricesZero();
	}

	unsigned CountMove = TObjectManager<TMoveProduct>::GetList().size();
	for(unsigned i=0; i < CountMove; i++) {
		TObjectManager<TMoveProduct>::GetList()[i]->CalcObjectDebts();
	}
