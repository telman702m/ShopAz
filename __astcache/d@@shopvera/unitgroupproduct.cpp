...
//			*TmpGroupProd = *(TGroupProduct::VGroupProduct[FormGroupList->MyListView->ItemIndex]);
			TmpGroupProd = TObjectManager<TGroupProduct>::GetList()[FormGroupList->MyListView->ItemIndex];
...
void __fastcall TFormGroupProduct::LoadGroupsToCombo(void)
{
	...
	for(unsigned i=0; i < TObjectManager<TGroupProduct>::GetList().size(); i++) {
		FormProduct->ComboBoxGroup->Items->Add(TObjectManager<TGroupProduct>::GetList()[i]->Name);
	}
}
//---------------------------------------------------------------------------
