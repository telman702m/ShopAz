...
{
	return TObjectManager<TProduct>::FindById(id);
}
//---------------------------------------------------------------------------
bool __fastcall TProduct::SaveDataToFile(char *FileNmae)
{
	...
	for(unsigned i=0; i < TObjectManager<TProduct>::GetList().size(); i++) {
		fwprintf(fs, L"'%d'\t'%d'\t'%ls'\t'%ls'\t'%ls'\t'%d'\t'%d'\t'%d'\t'%d'\t'%d'\t'%d'\t'%d'\t'%d'\t'%d'\t'%ls'\t'%d'\n",
					 TObjectManager<TProduct>::GetList()[i]->id,
					 TObjectManager<TProduct>::GetList()[i]->IdGroup,
					 TObjectManager<TProduct>::GetList()[i]->Code.w_str(),
					 TObjectManager<TProduct>::GetList()[i]->Name.w_str(),
					 TObjectManager<TProduct>::GetList()[i]->Unit.w_str(),
					 TObjectManager<TProduct>::GetList()[i]->WholeUnit,
					 TObjectManager<TProduct>::GetList()[i]->PurchaseCash.GetMoney(),
					 TObjectManager<TProduct>::GetList()[i]->PurchaseCash.GetCent(),
					 TObjectManager<TProduct>::GetList()[i]->SellingPrice.GetMoney(),
					 TObjectManager<TProduct>::GetList()[i]->SellingPrice.GetCent(),
					 TObjectManager<TProduct>::GetList()[i]->DollarPrice.GetMoney(),
					 TObjectManager<TProduct>::GetList()[i]->DollarPrice.GetCent(),
					 TObjectManager<TProduct>::GetList()[i]->WholePrice.GetMoney(),
					 TObjectManager<TProduct>::GetList()[i]->WholePrice.GetCent(),
					 TUtills::ReplaceSymbol(TUtills::ReplaceSymbol(TObjectManager<TProduct>::GetList()[i]->Description.w_str(), L'\r', wchar_t(27)), L'\n', wchar_t(28)),
					 TObjectManager<TProduct>::GetList()[i]->Enable);
	}
	fclose(fs);
	return true;
}
...
//	int counter = 0;
	...
	while(Offset < FileSize) {
		...
		TObjectManager<TProduct>::GetList().push_back(TmpProduct);
//		counter++;
	}
...
{
	TObjectManager<TProduct>::GetList().clear();
