...
void __fastcall TSelectProduct::SaveAtObjectToDb(TMyFDQuery *FDQuery)
{
	...
	ExecSQL_Managed<TSelectProduct>(FDQuery, uQuery, NameTableDB, TabDB::TB_SELECTP, TRecordType::RT_UPDATE, LOGS::LG_UPDATE);
}
...
void __fastcall TSelectProduct::DeleteObjectFromDb(TMyFDQuery *FDQuery)
{
	...
	ExecSQL_Managed<TSelectProduct>(FDQuery, uQuery, NameTableDB, TabDB::TB_SELECTP, TRecordType::RT_DELETE, LOGS::LG_DELETE);
}
...
bool __fastcall TSelectProduct::SaveDataToFile(char *FileNmae)
{
	...
	for(unsigned i=0; i < TObjectManager<TSelectProduct>::GetList().size(); i++) {
		fwprintf(fs, L"'%d'\t'%d'\t'%s'\t'%d'\t'%d'\t'%d'\t'%d'\t'%d'\t'%d'\t'%d'\t'%d'\n",
					 TObjectManager<TSelectProduct>::GetList()[i]->id,
					 TObjectManager<TSelectProduct>::GetList()[i]->idMove,
					 TObjectManager<TSelectProduct>::GetList()[i]->SelectDateTime.FormatString(L"dd.mm.yyyy hh:nn:ss").w_str(),
					 TObjectManager<TSelectProduct>::GetList()[i]->idProduct,
					 TObjectManager<TSelectProduct>::GetList()[i]->IncomingPrice.GetMoney(),
					 TObjectManager<TSelectProduct>::GetList()[i]->IncomingPrice.GetCent(),
					 TObjectManager<TSelectProduct>::GetList()[i]->Price.GetMoney(),
					 TObjectManager<TSelectProduct>::GetList()[i]->Price.GetCent(),
					 TObjectManager<TSelectProduct>::GetList()[i]->Count.GetWhole(),
					 TObjectManager<TSelectProduct>::GetList()[i]->Count.GetFractional(),
					 TObjectManager<TSelectProduct>::GetList()[i]->bDeleted);
	}
...
//		int indexMove = GetArrayIndexById((vector <TBaseData*> *)&TMoveProduct::VMoveProduct, TmpSelectProduct->idMove);
		...
		if(MoveProductTmp != NULL) {
			...
			if(TmpSelectProduct->bDeleted == MoveProductTmp->bDeleted) {
				MoveProductTmp->VSelectedProd.push_back(TmpSelectProduct);
				MoveProductTmp->CalcTotalPrice();
			}
		} else {
...
{
	TObjectManager<TSelectProduct>::GetList().clear();
...
//	int IndexMove = TMoveProduct::GetArrayIndexById((vector <TBaseData*> *)&TMoveProduct::VMoveProduct, idMove);
	...
	if(MoveProductTmp != NULL) {
		MoveProductTmp->VSelectedProd.push_back(this);
		MoveProductTmp->CalcTotalPrice();
...
{
	unsigned Count = TObjectManager<TSelectProduct>::GetList().size();

	for(unsigned i=0; i < Count; i++) {
		if(TObjectManager<TSelectProduct>::GetList()[i] == SelProd) {
			TObjectManager<TSelectProduct>::GetList().erase(TObjectManager<TSelectProduct>::GetList().begin() + i);
			return;
		}
	}
}
...
{
	int CountSelectProd = TObjectManager<TSelectProduct>::GetList().size();
	int CountExcess = 0;
