...
void __fastcall TSynchronize::ApplySinc(void)
{
	...
	for(unsigned i=0; i < VSynchronize.size(); i++) {
		switch(VSynchronize[i]->TableDB) {
			case TabDB::TB_GROUP:
				if(TmpGroupProd->ApplyBaseSyncronize_Managed<TGroupProduct>(FormShop->FDQuery1, VSynchronize[i], FormGroupList)) {
				...
				break;
			case TabDB::TB_PROD:
				if(TmpProduct->ApplyBaseSyncronize_Managed<TProduct>(FormShop->FDQuery1, VSynchronize[i], FormProductList)) {
					TmpProduct = new TProduct();
				}
				break;
			case TabDB::TB_BUYER:
				if(TmpBuyer->ApplyBaseSyncronize_Managed<TBuyer>(FormShop->FDQuery1, VSynchronize[i], FormBuyerList)) {
					TmpBuyer = new TBuyer();
				}
				break;
			case TabDB::TB_PROVIS:
				if(TmpProvisioner->ApplyBaseSyncronize_Managed<TProvisioner>(FormShop->FDQuery1, VSynchronize[i], FormProvisionerList)) {
					TmpProvisioner = new TProvisioner();
				}
				break;
			case TabDB::TB_SELECTP:
				if(TmpSelectProduct->ApplyBaseSyncronize_Managed<TSelectProduct>(FormShop->FDQuery1, VSynchronize[i], NULL)) {
					TmpSelectProduct = new TSelectProduct();
				}
				bRecalc = true;
				break;
			case TabDB::TB_MOVEP:
				if(TmpMoveProduct->ApplyBaseSyncronize_Managed<TMoveProduct>(FormShop->FDQuery1, VSynchronize[i], FormProductList)) {
					TmpMoveProduct = new TMoveProduct();
				}
				bRecalc = true;
				break;
