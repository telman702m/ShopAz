...
void __fastcall TFormBuyerList::DeleteItem(void)
{
	if(MyListView->ItemIndex != -1) {
		...
		for(unsigned i=0; i < TObjectManager<TMoveProduct>::GetList().size(); i++) {
			if(TObjectManager<TMoveProduct>::GetList()[i]->SourcePerson == TObjectManager<TBuyer>::GetList()[MyListView->ipp[MyListView->ItemIndex]] ||
			   TObjectManager<TMoveProduct>::GetList()[i]->TargetPerson == TObjectManager<TBuyer>::GetList()[MyListView->ipp[MyListView->ItemIndex]]) {
					uTmp.sprintf(wBuyerExistsMessage[iLang], TObjectManager<TMoveProduct>::GetList()[i]->DateTime.FormatString(wDateTimeFormat).w_str());
		...
		if(Application->MessageBox(wDeleteMessage[iLang], uCaptionWarning[iLang], MB_YESNO) == IDYES) {
			...
			TObjectManager<TBuyer>::GetList()[MyListView->ipp[MyListView->ItemIndex]]->DeleteObjectFromDb(FormShop->FDQuery1);;
