...
void __fastcall TFormGroupList::DeleteItem(void)
{
	if(MyListView->ItemIndex != -1) {
		...
		for(unsigned i=0; i < TObjectManager<TProduct>::GetList().size(); i++) {
			if(TObjectManager<TProduct>::GetList()[i]->GroupProd == TObjectManager<TGroupProduct>::GetList()[MyListView->ipp[MyListView->ItemIndex]]) {
		...
		if(Application->MessageBox(wDeleteMessage[iLang], uCaptionWarning[iLang], MB_YESNO) == IDYES) {
			...
			TObjectManager<TGroupProduct>::GetList()[MyListView->ipp[MyListView->ItemIndex]]->DeleteObjectFromDb(FormShop->FDQuery1);
