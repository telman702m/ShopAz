...
{
	CountReport = TObjectManager<TShopUnits>::GetList().size();
...
void __fastcall TReport::RestoreOrderShopsName(void)
{
	for(unsigned i=0; i < TObjectManager<TShopUnits>::GetList().size(); i++) {
		Reports[i]->NameUnit = TObjectManager<TShopUnits>::GetList()[i]->Name;
	}
}
//---------------------------------------------------------------------------
void __fastcall TReport::CalcReport(void)
{
	...
	vector <TMoveProduct *> &VMoves = TObjectManager<TMoveProduct>::GetList();   // alias
	int iSource, iTarget;
...
/*
	...
	for(unsigned i=0; i < Count; i++) {
		for(unsigned j=0; j < TObjectManager<TShopUnits>::GetList().size(); j++) {
			if(TObjectManager<TProduct>::GetList()[i]->CountBalance[j] > 0) {
				Reports[j]->BalanceProductCost += TObjectManager<TProduct>::GetList()[i]->PurchaseCash * TObjectManager<TProduct>::GetList()[i]->CountBalance[j];
				Reports[j]->BalanceProductPrice += TObjectManager<TProduct>::GetList()[i]->SellingPrice * TObjectManager<TProduct>::GetList()[i]->CountBalance[j];
			}
		}
	}
