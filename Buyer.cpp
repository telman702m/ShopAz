...
/*const int CountFieldsBuyer = 12;
wchar_t *BuyerDbFields[CountFieldsBuyer][2] =
   {{L"id",               L" Int(11) Not Null Auto_increment, "},
	{L"Surname",       	  L" Char(40), "},
	{L"Name",	          L" Char(40), "},
	{L"Patronymic",       L" Char(40), "},
	{L"Organization",     L" Char(80), "},
	{L"Address",          L" Char(80), "},
	{L"Phone1",           L" Char(24), "},
	{L"Phone2",           L" Char(24), "},
	{L"Description",      L" Varchar(368), "},
	{L"Enable",           L" SmallInt(1), "},
	{L"Deleted",		  L" Smallint(1) "},
	{L"LastUpdate",	  	  L" TIMESTAMP CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, "}
	};  */

vector <TBuyer*> TBuyer::VBuyers;
...
{
	VBaseData = (vector <TBaseData*> *)&VBuyers;
	Clear();
}
//---------------------------------------------------------------------------
...
void __fastcall TBuyer::InsertAtObjectToDb(TMyFDQuery *FDQuery)
{
	...
	ExecSQL(FDQuery, FieldsValues, Count, NameTableDB, BuyerFieldsTableDB[0]->Field, TabDB::TB_BUYER);
}
...
void __fastcall TBuyer::SaveAtObjectToDb(TMyFDQuery *FDQuery)
{
	...
	ExecSQL(FDQuery, uQuery, NameTableDB, TabDB::TB_BUYER, TRecordType::RT_UPDATE, LOGS::LG_UPDATE);
}
...
void __fastcall TBuyer::DeleteObjectFromDb(TMyFDQuery *FDQuery)
{
	...
	ExecSQL(FDQuery, uQuery, NameTableDB, TabDB::TB_BUYER, TRecordType::RT_DELETE, LOGS::LG_DELETE);
}
...
bool __fastcall TBuyer::SaveDataToFile(char *FileNmae)
{
	...
	for(unsigned i=0; i < VBuyers.size(); i++) {
		fwprintf(fs, L"'%d'\t'%ls'\t'%ls'\t'%ls'\t'%ls'\t'%ls'\t'%ls'\t'%ls'\t'%ls'\t'%d'\n",
					 VBuyers[i]->id,
					 VBuyers[i]->Surname.w_str(),
					 VBuyers[i]->Name.w_str(),
					 VBuyers[i]->Patronymic.w_str(),
					 VBuyers[i]->Organization.w_str(),
					 VBuyers[i]->Address.w_str(),
					 VBuyers[i]->Phone1.w_str(),
					 VBuyers[i]->Phone2.w_str(),
					 TUtills::ReplaceSymbol(TUtills::ReplaceSymbol(VBuyers[i]->Description.w_str(), L'\r', wchar_t(27)), L'\n', wchar_t(28)),
					 VBuyers[i]->Enabled);
	}
	fclose(fs);
	return true;
		...
		try {
			...
			while (!FDQuery->Eof) {
				...
				if(bLoadAll) {
					...
					if(iDel == 1) {
						if(iIndex != -1) {
							VBuyers.erase(VBuyers.begin() + iIndex);
						}
					} else {
						if(iIndex == -1) {
							int InsertIndex = TmpBuyer->GetArrayIndexById(true);
							VBuyers.insert(VBuyers.begin()+InsertIndex, TmpBuyer);
						}
