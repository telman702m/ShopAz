...
/*const int CountFieldsGroup = 7;
wchar_t *GroupDbFields[CountFieldsGroup][2] =
   {{L"id",              L" Int(11) Not Null Auto_increment, "},
	{L"GroupCode",       L" Char(20), "},
	{L"Name",            L" Char(30), "},
...
{
	VBaseData = (vector <TBaseData*> *)&VGroupProduct;
	Clear();
}
//---------------------------------------------------------------------------
...
void __fastcall TGroupProduct::InsertAtObjectToDb(TMyFDQuery *FDQuery)
{
	...
	ExecSQL(FDQuery, FieldsValues, Count, NameTableDB, GroupProdFieldsTableDB[0]->Field, TabDB::TB_GROUP);
}
...
void __fastcall TGroupProduct::SaveAtObjectToDb(TMyFDQuery *FDQuery)
{
	...
	ExecSQL(FDQuery, uQuery, NameTableDB, TabDB::TB_GROUP, TRecordType::RT_UPDATE, LOGS::LG_UPDATE);
}
...
void __fastcall TGroupProduct::DeleteObjectFromDb(TMyFDQuery *FDQuery)
{
	...
	ExecSQL(FDQuery, uQuery, NameTableDB, TabDB::TB_GROUP, TRecordType::RT_DELETE, LOGS::LG_DELETE);
}
...
{
	unsigned Count = VGroupProduct.size();

	for(unsigned i=0; i < Count; i++) {
		if(VGroupProduct[i]->Name == Name) {
			return VGroupProduct[i];
		}
	}
	return NULL;
}
	...
	while(!bSuccess) {
		try {
			...
			while (!FDQuery->Eof)
			{
				...
				VGroupProduct.push_back(TmpGroupProd);
				FDQuery->Next();
