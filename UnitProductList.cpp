...
void __fastcall TFormProductList::FormCreate(TObject *Sender)
{
	...
	SizeColumnsData = CountStatColumn + TShopUnits::VShopUnits.size();
...
void __fastcall TFormProductList::FillShopsName(void)
{
	for(unsigned i=0; i < TShopUnits::VShopUnits.size(); i++) {
		if(i==0) {
			...
			ColumnsData[i + CountStatColumn].Caption[0] = TShopUnits::VShopUnits[TShopUnits::upp[i]]->Name;
			ColumnsData[i + CountStatColumn].Caption[1] = TShopUnits::VShopUnits[TShopUnits::upp[i]]->Name;
		}
...
void __fastcall TFormProductList::DeleteItem(void)
{
	if(MyListView->ItemIndex != -1) {
		...
		for(unsigned i=0; i < TMoveProduct::VMoveProduct.size(); i++) {
			for(unsigned j=0; j < TMoveProduct::VMoveProduct[i]->VSelectedProd.size(); j++) {
				if(TMoveProduct::VMoveProduct[i]->VSelectedProd[j]->Product == TProduct::VProduct[MyListView->ipp[MyListView->ItemIndex]]) {
					uTmp.sprintf(wDeleteProduct[iLang], TMoveProduct::VMoveProduct[i]->DateTime.FormatString(wDateTimeFormat).w_str());
		...
		if(Application->MessageBox(wDeleteMessage[iLang], uCaptionWarning[iLang], MB_YESNO) == IDYES) {
			...
			TProduct::VProduct[MyListView->ipp[MyListView->ItemIndex]]->DeleteObjectFromDb(FormShop->FDQuery1);
...
void __fastcall TFormProductList::LoadGroupsToComboFilter(void)
{
	...
	for(unsigned i=0; i < TGroupProduct::VGroupProduct.size(); i++) {
		ComboBoxFilter->Items->Add(TGroupProduct::VGroupProduct[i]->Name);
	}
...
bool __fastcall TFormProductList::FilteredProduct(int i)
{
	if(ComboBoxFilter->ItemIndex != 0) {
		if(!(ComboBoxFilter->Text == TProduct::VProduct[i]->GroupProd->Name)) {
			return false;
        }
	}

	if(FormProductList->EditFind->Text.Length() > 0) {
		if(!((TProduct::VProduct[i]->Name.UpperCase().Pos(FormProductList->EditFind->Text.UpperCase()) ||
			 TProduct::VProduct[i]->Code.UpperCase().Pos(FormProductList->EditFind->Text.UpperCase())) > 0)) {
			return false;
		}
	}

	if(!CheckBoxShowEmpty->Checked) {
		if(!(TProduct::VProduct[i]->CountBalance[TShopUnits::upp[0]] > 0)) {
			return false;
        }
	}
	return true;
...
void __fastcall TFormProductList::ButtonPrintClick(TObject *Sender)
{
	...
	if(ComboBoxFilter->ItemIndex == 0) {
		...
		IdGroup = TGroupProduct::VGroupProduct[ComboBoxFilter->ItemIndex - 1]->id;
	}
...
void __fastcall TFormProductList::ButtonExportToExcelClick(TObject *Sender)
{
	...
	FormProgressBar->Update();
	FormProgressBar->ProgressBar1->Max = TProduct::VProduct.size();
	...
	for(unsigned i = 0; i < TProduct::VProduct.size(); i++) {
		...
		if(FilteredProduct(i)) {
			for(int j = 0; j < MyListView->iCountColumn; j++) {
				...
				case 0:
					// Group
					WriteToExcelCell(myExcel, TProduct::VProduct[i]->GroupProd->Name, MyListView->ColumnsData[j].Visible);
					break;
				case 1:
					// TProduct Code
					WriteToExcelCell(myExcel, TProduct::VProduct[i]->Code, MyListView->ColumnsData[j].Visible);
					break;
				case 2:
					// TProduct Name
					WriteToExcelCell(myExcel, TProduct::VProduct[i]->Name, MyListView->ColumnsData[j].Visible);
					break;
				case 3:
					// TProduct Unit
					WriteToExcelCell(myExcel, TProduct::VProduct[i]->Unit, MyListView->ColumnsData[j].Visible);
					break;
				case 4:
					// Cash Purchase
					WriteToExcelCell(myExcel, TProduct::VProduct[i]->PurchaseCash.GetValue(), MyListView->ColumnsData[j].Visible);
					break;
				case 5:
					// Cash Purchase in $
					WriteToExcelCell(myExcel, TProduct::VProduct[i]->DollarPrice.GetValue(), MyListView->ColumnsData[j].Visible);
					break;
				case 6:
					// Selling Purchase
					WriteToExcelCell(myExcel, TProduct::VProduct[i]->SellingPrice.GetValue(), MyListView->ColumnsData[j].Visible);
					break;
				case 7:
					// Whole Selling Purchase
					WriteToExcelCell(myExcel, TProduct::VProduct[i]->WholePrice.GetValue(), MyListView->ColumnsData[j].Visible);
					break;
				case 8:
					// Cash Balance
					WriteToExcelCell(myExcel, TProduct::VProduct[i]->CountBalance[TShopUnits::upp[0]].GetValue(), MyListView->ColumnsData[j].Visible);
					break;
				}
			}
			CountRows++;
