...
void __fastcall TFormShop::FormShow(TObject *Sender)
{
	...
	if(FormUserStatus->bSuccess) {
		for(unsigned i=0; i < TShopUnits::VShopUnits.size(); i++) {
			ComboBoxUShops->Items->Add(TShopUnits::VShopUnits[i]->Name);
		}
		...
		if(bValidOrder && TProduct::VProduct.size() > 0) {
			bValidOrder = TProduct::VProduct[0]->CheckSortById();
		}

		TBuyer::LoadFromDB(FDQuery1, bDataLoadType);
		if(bValidOrder && TBuyer::VBuyers.size() > 0) {
			bValidOrder = TBuyer::VBuyers[0]->CheckSortById();
		}

		TProvisioner::LoadFromDB(FDQuery1, bDataLoadType);
		if(bValidOrder && TProvisioner::VProvisioners.size() > 0) {
			bValidOrder = TProvisioner::VProvisioners[0]->CheckSortById();
		}

		TMoveProduct::LoadFromDB(FDQuery1, bDataLoadType);
		if(bValidOrder && TMoveProduct::VMoveProduct.size() > 0) {
			bValidOrder = TMoveProduct::VMoveProduct[0]->CheckSortById();
		}

		TSelectProduct::LoadFromDB(FDQuery1, bDataLoadType);
		if(bValidOrder && TSelectProduct::VSelectProduct.size() > 0) {
			bValidOrder = TSelectProduct::VSelectProduct[0]->CheckSortById();
		}
...
bool __fastcall TFormShop::CheckAccess(void)
{
	...
	TMoveProduct *TmpMoveProd = TMoveProduct::VMoveProduct[index];
...
void __fastcall TFormShop::ToolButtonDeleteClick(TObject *Sender)
{
	...
	if(Application->MessageBox(uShopDeleteRecord[iLang], uCaptionWarning[iLang], MB_YESNO) == IDYES) {
		...
		FormMoveProduct->DeleteMoveProduct(TMoveProduct::VMoveProduct[MoveList->ipp[MoveList->ItemIndex]]);
