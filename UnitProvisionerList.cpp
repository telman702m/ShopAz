...
void __fastcall TFormProvisionerList::DeleteItem(void)
{
	if(MyListView->ItemIndex != -1) {
		...
		for(unsigned i=0; i < TMoveProduct::VMoveProduct.size(); i++) {
			if(TMoveProduct::VMoveProduct[i]->SourcePerson == TProvisioner::VProvisioners[MyListView->ipp[MyListView->ItemIndex]] ||
			   TMoveProduct::VMoveProduct[i]->TargetPerson == TProvisioner::VProvisioners[MyListView->ipp[MyListView->ItemIndex]]) {
					uTmp.sprintf(wProvisionerExistsMessage[iLang], TMoveProduct::VMoveProduct[i]->DateTime.FormatString(wDateTimeFormat).w_str());
		...
		if(Application->MessageBox(wDeleteMessage[iLang], uCaptionWarning[iLang], MB_YESNO) == IDYES) {
			...
			TProvisioner::VProvisioners[MyListView->ipp[MyListView->ItemIndex]]->DeleteObjectFromDb(FormShop->FDQuery1);;
